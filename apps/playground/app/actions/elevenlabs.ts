...
async function getStoredApiKey(): Promise<string> {
  ...
  if (!encryptedKey) {
    throw new Error('No ElevenLabs API key found. Please add your API key using the key icon in the header.');
  }
...
export async function createSecret(name: string, value: string): Promise<CreateSecretResponse> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      throw new Error(error.message || `Failed to create secret: ${response.status}`);
    } catch {
...
export async function createTool(toolConfig: any): Promise<CreateToolResponse> {
  ...
  console.log('[createTool] Starting with config:', JSON.stringify(toolConfig, null, 2));
  ...
  console.log('[createTool] Request body:', JSON.stringify(requestBody, null, 2));
  ...
  if (!response.ok) {
    ...
    try {
      const error = JSON.parse(errorText);
      throw new Error(error.message || `Failed to create tool: ${response.status}`);
    } catch {
...
export async function addToolWithSecrets(
  ...
  try {
    ...
    if (tool.required_secrets && secrets) {
      console.log('[addToolWithSecrets] Creating secrets for:', tool.required_secrets.map((s: any) => s.name));
      ...
      for (const requiredSecret of tool.required_secrets) {
        ...
        console.log('[addToolWithSecrets] Creating secret:', requiredSecret.name);
        ...
        console.log('[addToolWithSecrets] Secret created with ID:', secretResponse.secret_id);
      }
    }
    ...
    } else if (toolConfig.api_schema?.query_params_schema?.properties && 
               typeof toolConfig.api_schema.query_params_schema.properties === 'object' &&
               Object.keys(toolConfig.api_schema.query_params_schema.properties).length === 0) {
      // Remove if properties is empty
      delete toolConfig.api_schema.query_params_schema;
    }
    
    // 3. Transform request_body_schema properties from array to object
    if (toolConfig.api_schema?.request_body_schema?.properties && Array.isArray(toolConfig.api_schema.request_body_schema.properties)) {
      ...
      for (const prop of toolConfig.api_schema.request_body_schema.properties) {
        ...
        if (prop.dynamic_variable !== undefined && prop.dynamic_variable !== '') {
        ...
        if (prop.type === 'array' && prop.items) {
          ...
          if (prop.items.properties && Array.isArray(prop.items.properties)) {
            ...
            for (const itemProp of prop.items.properties) {
              ...
              if (itemProp.constant_value !== undefined && itemProp.constant_value !== '') {
                itemPropDef.constant_value = itemProp.constant_value;
              }
              if (itemProp.dynamic_variable !== undefined && itemProp.dynamic_variable !== '') {
      ...
      toolConfig.api_schema.request_body_schema = {
        ...
        properties,
        required,
        description: toolConfig.api_schema.request_body_schema.description || '',
      };
    }
    ...
    if (tool.required_secrets && secretIds) {
      ...
      console.log('[addToolWithSecrets] Updated request headers with secret IDs');
    ...
    console.log('[addToolWithSecrets] Tool created successfully:', toolResponse.id);
