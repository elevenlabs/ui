---
title: Speech Input
description: A compact, animated speech-to-text input component with real-time transcription using ElevenLabs Scribe.
featured: true
component: true
---
...
export default function Example() {
  return (
    <SpeechInput
      getToken={getToken}
      onChange={(event) => console.log(event.transcript)}
      onStop={(event) => console.log("Final:", event.transcript)}
    >
      <SpeechInputRecordButton />
...
export default function Example() {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <SpeechInput
        getToken={getToken}
        onStop={(event) => setValue((prev) => prev + " " + event.transcript)}
      >
        <SpeechInputRecordButton />
        <SpeechInputPreview />
        <SpeechInputCancelButton />
...
<SpeechInput
  getToken={getToken}
  onStop={(event) => console.log(event.transcript)}
>
  <SpeechInputRecordButton />
</SpeechInput>
```
...
| onChange                | `(event: SpeechInputEvent) => void` | -                      | Called when transcript changes                        |
| onStart                 | `(event: SpeechInputEvent) => void` | -                      | Called when recording starts                          |
| onStop                  | `(event: SpeechInputEvent) => void` | -                      | Called when recording stops                           |
| onCancel                | `(event: SpeechInputEvent) => void` | -                      | Called when recording is cancelled                    |
